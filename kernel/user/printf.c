...
static int writestr(const char *str, size_t len, int (*put)(int c, void *),
		    void *opaque)
{
	int nwritten = 0;
		     ...
		     int prec, int (*put)(int c, void *), void *opaque)
{
	...
	ret = writestr(prefix, prefixlen, put, opaque);
	if (ret < 0)
		goto write_error;
	...
	ret = writestr(num, numlen, put, opaque);
	if (ret < 0)
		goto write_error;
...
static int write_to_fd(int ch, void *opaque)
{
	struct printf_fd *fd = (struct printf_fd *)opaque;
	char c = ch;

	int res = write(fd->fd, &c, 1);
	if (res <= 0)
		return -1;
